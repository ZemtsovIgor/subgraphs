...
import { updatePairDayData, updateTokenDayData, updateTBCCFinanceDayData, updatePairHourData } from "./dayUpdates";
...
export function handleSync(event: Sync): void {
  ...
  let tbccFinance = TBCCFinanceFactory.load(FACTORY_ADDRESS);

  // reset factory liquidity by subtracting only tracked liquidity
  tbccFinance.totalLiquidityBNB = tbccFinance.totalLiquidityBNB.minus(pair.trackedReserveBNB as BigDecimal);
  ...
  tbccFinance.totalLiquidityBNB = tbccFinance.totalLiquidityBNB.plus(trackedLiquidityBNB);
  tbccFinance.totalLiquidityUSD = tbccFinance.totalLiquidityBNB.times(bundle.bnbPrice);
...
export function handleMint(event: Mint): void {
  ...
  tbccFinance.totalTransactions = tbccFinance.totalTransactions.plus(ONE_BI);
  ...
  updateTBCCFinanceDayData(event);
...
export function handleSwap(event: Swap): void {
  ...
  let tbccFinance = TBCCFinanceFactory.load(FACTORY_ADDRESS);
  tbccFinance.totalVolumeUSD = tbccFinance.totalVolumeUSD.plus(trackedAmountUSD);
  tbccFinance.totalVolumeBNB = tbccFinance.totalVolumeBNB.plus(trackedAmountBNB);
  tbccFinance.untrackedVolumeUSD = tbccFinance.untrackedVolumeUSD.plus(derivedAmountUSD);
  tbccFinance.totalTransactions = tbccFinance.totalTransactions.plus(ONE_BI);
  ...
  let tbccFinanceDayData = updateTBCCFinanceDayData(event);
  let token0DayData = updateTokenDayData(token0 as Token, event);
  let token1DayData = updateTokenDayData(token1 as Token, event);

  // swap specific updating
  tbccFinanceDayData.dailyVolumeUSD = tbccFinanceDayData.dailyVolumeUSD.plus(trackedAmountUSD);
  tbccFinanceDayData.dailyVolumeBNB = tbccFinanceDayData.dailyVolumeBNB.plus(trackedAmountBNB);
  tbccFinanceDayData.dailyVolumeUntracked = tbccFinanceDayData.dailyVolumeUntracked.plus(derivedAmountUSD);
  tbccFinanceDayData.save();
