...
export class IdIncrement extends Entity {
  constructor(id: string) {
    super();
    this.set("id", Value.fromString(id));
  }
  ...
  save(): void {
    let id = this.get("id");
    assert(id !== null, "Cannot save IdIncrement entity without an ID");
    assert(
      id.kind == ValueKind.STRING,
      "Cannot save IdIncrement entity with non-string ID. " +
        'Considering using .toHex() to convert the "id" to a string.'
    );
    store.set("IdIncrement", id.toString(), this);
  }

  static load(id: string): IdIncrement | null {
    return store.get("IdIncrement", id) as IdIncrement | null;
  }
  ...
  get autoIncrement(): BigInt {
    let value = this.get("autoIncrement");
    return value.toBigInt();
  }

  set autoIncrement(value: BigInt) {
    this.set("autoIncrement", Value.fromBigInt(value));
  }
}
